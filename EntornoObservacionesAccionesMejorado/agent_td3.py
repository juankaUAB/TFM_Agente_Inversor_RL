...
model = TD3("MlpPolicy", env, verbose = 1, learning_rate=0.0003, learning_starts=10000, buffer_size=100000, tensorboard_log="./TD3_stocktrading_tensorboard/", replay_buffer_class=ReplayBuffer)
eval_callback = EvalCallback(eval_env, best_model_save_path='./logs-TD3/',
                             log_path='./logs-TD3/', eval_freq=10000,
                             deterministic=True, render=False)
profit_callback = ProfitLoggingCallback()
callback = CallbackList([profit_callback, eval_callback])
model.learn(total_timesteps=200_000, log_interval=5, callback=callback, progress_bar=True, tb_log_name = "TD3_200k_lrStable")
