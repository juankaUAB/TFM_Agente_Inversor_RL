...
model = DDPG("MlpPolicy", env, verbose = 1, learning_rate=0.0003, learning_starts=10000, buffer_size=100000, tensorboard_log="./DDPG_stocktrading_tensorboard/", replay_buffer_class=ReplayBuffer, device="cuda")
eval_callback = EvalCallback(eval_env, best_model_save_path='./logs-DDPG/',
                             log_path='./logs-DDPG/', eval_freq=25000,
                             deterministic=True, render=False)
profit_callback = ProfitLoggingCallback()
callback = CallbackList([profit_callback, eval_callback])
model.learn(total_timesteps=2_000_000, log_interval=5, callback=callback, progress_bar=True, tb_log_name = "DDPG_2M_lrStable")
