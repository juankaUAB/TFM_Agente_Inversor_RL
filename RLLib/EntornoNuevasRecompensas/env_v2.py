...
class StockMarketEnv(gym.Env):
    def __init__(self, config=None):
        ...
        if len(config) > 0:
            ...
            data_file = r'C:\Users\Juanka\Desktop\Master Data Science\Trabajo final de Master\Desarrollo\RLLib\EntornoObservacionesAccionesMejorado\preprocessed_data_1d.csv'
            window_size = 6
            step = 'train'
    ...
    def step(self, action):
        ...
        if (action == 1 or action == 2) and (self.turbulence[self.current_step] > 0.8):  # Si decide comprar o vender en un mercado volátil
        ...
        if action == 0 and self.shares_held== 0 and self.current_step > 0:
            if self.prices[self.current_step] > self.prices[self.current_step-1]: # Leve recompensa si el precio baja y se compran acciones, penalización si el precio sube y no se compra.
                factor_multiplicativo_reward -= 0.05
            else:
                factor_multiplicativo_reward += 0.05
