...
model = A2C("MlpPolicy", env, verbose = 1, learning_rate = 0.0003, tensorboard_log="./a2c_stocktrading_tensorboard/", n_steps=2048, ent_coef=0.001, device="cuda")
eval_callback = EvalCallback(eval_env, best_model_save_path='./logs-A2C/',
                             log_path='./logs-A2C/', eval_freq=25000,
                             deterministic=True, render=False)
profit_callback = ProfitLoggingCallback()
callback = CallbackList([profit_callback, eval_callback])
model.learn(total_timesteps=200_000, log_interval=5, callback=callback, progress_bar=True, tb_log_name = "A2C_200k_lrStable_entCoefModif")
